...
print(starttime)
# wendangming
wendangming_path = 'classroom/LZW/X/1/wendangming.txt'
...
# CSI数据
bfee = Bfee.from_file("classroom/LZW/X/1/log.dat", model_name_encode="gb2312")
...
for i in range(len(frame_index)-1):
    ...
    data_name ='classroom_data_unit/LZW/X/gresture_X_location_1_' + str(i)
...
print(starttime)
# wendangming
wendangming_path = 'classroom/LZW/O/1/wendangming.txt'
wendangming = np.loadtxt(wendangming_path,dtype=int,delimiter='****')   
# 帧序号，手势序号，时间戳  
frame_index, gesture_index, timestamps= np.split(wendangming, (1,2), axis=1) #pylint: disable=unbalanced-tuple-unpacking
# print(frame_index)
print(datetime.datetime.now())
# CSI数据
bfee = Bfee.from_file("classroom/LZW/O/1/log.dat", model_name_encode="gb2312")
# print(len(bfee.dicts))
# print(len(bfee.all_csi))
# 数据分割保存
print(datetime.datetime.now())
for i in range(len(frame_index)-1):
    data = bfee.dicts[frame_index[i][0]:frame_index[i+1][0]]
    data_name ='classroom_data_unit/LZW/O/gresture_O_location_1_' + str(i)
    #print(data_name,data)
    np.save(data_name, data)
print(datetime.datetime.now())
#* 记录程序运行时间，结束时间
...
print(starttime)
# wendangming
wendangming_path = 'classroom/LZW/PO/1/wendangming.txt'
wendangming = np.loadtxt(wendangming_path,dtype=int,delimiter='****')   
# 帧序号，手势序号，时间戳  
frame_index, gesture_index, timestamps= np.split(wendangming, (1,2), axis=1) #pylint: disable=unbalanced-tuple-unpacking
# print(frame_index)
print(datetime.datetime.now())
# CSI数据
bfee = Bfee.from_file("classroom/LZW/PO/1/log.dat", model_name_encode="gb2312")
# print(len(bfee.dicts))
# print(len(bfee.all_csi))
# 数据分割保存
print(datetime.datetime.now())
for i in range(len(frame_index)-1):
    data = bfee.dicts[frame_index[i][0]:frame_index[i+1][0]]
    data_name ='classroom_data_unit/LZW/PO/gresture_PO_location_1_' + str(i)
    #print(data_name,data)
    np.save(data_name, data)
print(datetime.datetime.now())
#* 记录程序运行时间，结束时间
...
gresture_O_location_1_6 = np.load('classroom_data_unit/LJP/X/gresture_X_location_1_6.npy',allow_pickle=True)
